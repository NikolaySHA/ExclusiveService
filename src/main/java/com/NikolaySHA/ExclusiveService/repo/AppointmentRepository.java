...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
    ...
    @Query("SELECT a FROM Appointment a WHERE (:date IS NULL OR a.date >= :date) " +
            "AND (:licensePlate IS NULL OR a.car.licensePlate LIKE %:licensePlate%) " +
            "AND (:make IS NULL OR a.car.make LIKE %:make%) " +
            "AND (:customer IS NULL OR a.user.name LIKE %:customer%) " +
            "AND (:status IS NULL OR a.status = :status)")
    Page<Appointment> searchAppointmentsPaginated(@Param("date") LocalDate date,
